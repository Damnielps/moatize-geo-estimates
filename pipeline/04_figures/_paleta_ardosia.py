...
Vendorizado sem alteração de `ardosia-brand-guidelines/scripts/palette.py` (skill
pessoal do autor, fora do repositório) para que `pipeline/04_figures/` não dependa
de um caminho fora do controle de versão. Fonte normativa: `references/palette.md`
e `SKILL.md` da mesma skill. Qualquer mudança de paleta deve ser replicada aqui
manualmente e registrada em `docs/ADR/`.

Uso:
    from _paleta_ardosia import apply_ardosia, ARDOSIA, VIZ_CATEGORICAL, emphasis
    apply_ardosia()
"""
